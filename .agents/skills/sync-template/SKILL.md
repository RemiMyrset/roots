...
`pnpm sync:template` stages the template's version of the shared mechanics: CI
workflows, labels, the agent-task issue and PR templates, docs generators and
checkers, the guard, sync, docs, and gate test-suites, agent hooks, rules,
skills, and output styles, `docs/template/`, and the script itself. It records
the sync point in `.template-sync.json` and prints two follow-ups a file copy
cannot carry: the template commits since the last sync (breaking ones marked
`!`) and the `package.json` scripts that differ. Recipe and contract:
`docs/template/docs-toolchain.md` and `docs/template/sync-template.md`.
...
1. Bootstrap if needed. If `scripts/sync-template.mts` is missing, or there is
   ...
   Overwriting the old one is fine; the script exempts itself from its own dirty
   check. The `sync:template` script shows up as a missing follow-up on a first
   run:
