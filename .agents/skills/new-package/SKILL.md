...
4. Create `vitest.config.ts` — copy `packages/example-package/vitest.config.ts` if it
...
5. Create `src/index.ts` plus a sibling `test/index.test.ts` with at least one
   ...
   extensions, so the test imports `../src/index.ts`. If this replaces a sample —
   `packages/example-package`, or `apps/example-app` (which depends on the
   package) — delete or rename it in the same change and fix the other's
   dependency.
